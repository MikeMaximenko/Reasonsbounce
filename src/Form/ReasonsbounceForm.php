...
class ReasonsbounceForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ContactFormInterface $contact_form = NULL) {    
    ...
    $form['function'] = array(
      '#type' => 'radios',
      '#title' => t('Select js event '),
      '#description' => t('Choose what type of event will be triggered.'),
      '#default_value' => $contact_form->getThirdPartySetting('reasonsbounce', 'function'),
      '#options' => array(
        'onbeforeunload' => t('onbeforeunload'),
        'onload' => t('onload'),
      ),
      '#wysiwyg' => FALSE,
      '#required' => TRUE
    ...
    $form['message'] = array(
      ...
      '#description' => t('Enter only if you chosen onbeforeunload js event.'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->contact_form->setThirdPartySetting('reasonsbounce', 'function', $form_state->getValue('function'));
